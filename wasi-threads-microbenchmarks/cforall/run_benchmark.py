...
# Array of benchmarks
benchmarks = ["mutexC", "schedint", "semaphore", "tsd", "tls"]
    ...
    for benchmark in benchmarks:
        ...
        with open(timecsv_file, "w") as time_file, open(rsscsv_file, "w") as rss_file:
            ...
            for threads in num_threads:
                ...
                command = [f"./{benchmark}/main", str(threads)]
                ...
                # Run naive with musl
                command = [f"./{benchmark}/main.musl", str(threads)]
                results = run_bench(command)
                ...
                # Run wasmtime.
                command = [wasmtime, "-S", "threads", f"{benchmark}/main.wasm", str(threads)]
                results = run_bench(command)
                ...
                # Run iwasm.
                command = [iwasm, f"--max-threads={threads + 1}", f"{benchmark}/main.wasm", str(threads)]
                results = run_bench(command)
                ...
                # Run wasmer.
                command = [wasmer, f"{benchmark}/main.wasm", str(threads)]
                results = run_bench(command)
