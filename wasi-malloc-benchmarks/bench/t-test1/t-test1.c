...
static void bin_alloc(struct bin *m, size_t size, unsigned r)
{
	...
	if (r < 4)
	{
		...
		//WASM doesn't support memalign
		// m->ptr = memalign(sizeof(int) << r, size);

		// Instead, use the aligned_alloc function, which is a standard C function available since C11.
		// Calculate the alignment
		size_t alignment = sizeof(int) << r;
		// The size must be a multiple of the alignment
		size_t adjusted_size =  size + alignment - 1 & -alignment;
		
		m->ptr = aligned_alloc(alignment, adjusted_size);
