  ...
  string name;                        /* name of parameter */
{
   ...
   // MODIFIED BY JULEN BOHOYO: error: WASM doen't support gets, updated to fgets
   //gets(buf);
   FILE* stdin_stream = stdin; // Guardamos la referencia al flujo de entrada estándar

   if (fgets(buf, sizeof(buf), stdin_stream) != NULL) {
      // Eliminar el carácter de nueva línea del final del buffer si está presente
      if (buf[strlen(buf) - 1] == '\n')
         buf[strlen(buf) - 1] = '\0';
   } else {
      // Manejo del error de lectura de entrada
