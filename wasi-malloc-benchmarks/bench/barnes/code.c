...
{
   printf("\033[1;31mEntra(Pranset)\033[0m\n");
   int proc;
   A = 1;
   B = 0;
   randx = (A*seed+B) & MASK;
...
{
   // MODIFIED BY JULEN BOHOYO: error: non-thread-local declaration of 'errno' follows thread-local declaration
   #ifdef WASM
...
string argv[];
{
  ...
  printf("Run this as\n  BARNES < input \n for default values\n");
   ...
   ANLinit();
   initparam(argv, defv);
   printf("\033[1;31mAntes\033[0m\n");
   startrun();
   printf("\033[1;31mPasa\033[0m\n");
   initoutput();
   tab_init();
   Global->tracktime = 0;
   ...
   {time_t time(); (Global->computestart) = time(0);};
   printf("COMPUTESTART  = %12u\n",Global->computestart);
   SlaveStart();

   {time_t time(); (Global->computeend) = time(0);};
...
ANLinit()
{
   ...
   if (Global==NULL){
      ...
      printf("\033[1;31mNo initialization for Global\033[0m\n");
   } 
   {;};
   {;};
   {;};
...
startrun()
{
   ...
   if (*infile != NULL) {
      ...
      if (nbody < 1) {
         ...
         printf("\033[1;31mstartrun: absurd nbody\033[0m\n");
      }
      seed = getiparam("seed");
   }
   ...
   Local[0].nstep = 0;
   printf("\033[1;32mAntes(Startrun)\033[0m\n");
   pranset(2);
   printf("\033[1;31mLlega2(Startrun)\033[0m\n");
   testdata();
   printf("\033[1;31mLlega3(Startrun)\033[0m\n");
   setbound();
   printf("\033[1;31mLlega4(Startrun)\033[0m\n");
...
testdata()
{
   ...
   if (bodytab == NULL) {
      ...
      printf("\033[1;31mtestdata: not enuf memory\033[0m\n");
   }
   rsc = 9 * PI / 16;
   vsc = sqrt(1.0 / rsc);
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
   statistics that one is measuring about the parallel execution */
    ...
    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {time_t time(); (treebuildstart) = time(0);};
    }
    ...
    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {time_t time(); (treebuildend) = time(0);};
    ...
    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {time_t time(); (partitionstart) = time(0);};
    }
...
/*     B*RRIER(Global->Barcom,NPROC); */
    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {time_t time(); (partitionend) = time(0);};
        Global->partitiontime += partitionend - partitionstart;
    }

    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {time_t time(); (forcecalcstart) = time(0);};
    }

    ComputeForces(ProcessId);

    if ((ProcessId == 0) && (Local[ProcessId].nstep >= 2)) {
        {time_t time(); (forcecalcend) = time(0);};
   ...
   if (Type(mycell) == LEAF) {
	 ...
	 if (work >= Local[ProcessId].workMin - .1) {
	    if((Local[ProcessId].mynbody+2) > maxmybody) {
          ...
          printf("\033[1;31mfind_my_bodies: Processor %d needs more than %d bodies; increase fleaves\033[0m\n",ProcessId, maxmybody);
	    }
